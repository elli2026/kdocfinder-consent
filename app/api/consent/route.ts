...
function generateEmail(p: { consent_id: string; name: string; nationality: string; language: string; created_at: string; expires_at: string }) {
  ...
  const t: Record<string, any> = {
    kr: {
      ...
      notice: '본 콘텐츠는 K-DocFinder 플랫폼 홍보 목적으로만 사용되며, 특정 병원의 직접 홍보에는 사용되지 않습니다.',
    ...
    en: {
      ...
      notice: 'This content is used solely for K-DocFinder platform promotion and will NOT be used for direct promotion of specific hospitals.',
    ...
    ja: {
      ...
      notice: '本コンテンツはK-DocFinderプラットフォームのプロモーション目的のみに使用されます。',
    ...
    'zh-cn': {
      ...
      notice: '此内容仅用于K-DocFinder平台推广，不会用于特定医院的直接宣传。',
      withdraw: '如需撤回同意，请联系：',
    },
    'zh-tw': {
      ...
      notice: '此內容僅用於K-DocFinder平台推廣，不會用於特定醫院的直接宣傳。',
      withdraw: '如需撤回同意，請聯繫：',
    },
  };

  const c = t[p.language] || t.en;
  const itemsHtml = c.items.map((i: string) => '<div style="font-size:14px;color:#333;padding:8px 0;border-bottom:1px solid #eee;">' + i + '</div>').join('');

  return '<!DOCTYPE html><html><head><meta charset="UTF-8"></head><body style="margin:0;padding:0;background:#f5f7fa;font-family:system-ui,sans-serif;"><div style="max-width:520px;margin:0 auto;padding:32px 16px;"><div style="background:#fff;border-radius:16px;overflow:hidden;box-shadow:0 2px 12px rgba(0,0,0,0.06);"><div style="background:linear-gradient(135deg,#003CB3,#0057FF,#3388FF);padding:28px 24px;text-align:center;"><div style="font-size:24px;font-weight:800;color:#fff;letter-spacing:-0.3px;">K-DocFinder</div><div style="font-size:14px;color:rgba(255,255,255,0.8);margin-top:6px;">' + c.title + '</div></div><div style="padding:28px 24px;"><p style="font-size:16px;color:#1a1f36;margin:0 0 6px;font-weight:600;">' + c.greeting + '</p><p style="font-size:14px;color:#666;margin:0 0 20px;">' + c.body + '</p><div style="background:#f7f8fa;border-radius:12px;padding:16px 20px;margin-bottom:20px;">' + itemsHtml + '</div><div style="background:#f0f5ff;border-left:3px solid #0057FF;border-radius:0 8px 8px 0;padding:14px 16px;margin-bottom:20px;"><p style="font-size:13px;color:#0052CC;margin:0;line-height:1.6;">' + c.notice + '</p></div><p style="font-size:13px;color:#888;margin:0 0 10px;">' + c.withdraw + '</p><a href="mailto:kdocfinder@gmail.com" style="display:inline-block;padding:10px 24px;background:#f0f0f0;border-radius:8px;font-size:14px;color:#0057FF;text-decoration:none;font-weight:600;">kdocfinder@gmail.com</a></div><div style="border-top:1px solid #f0f0f0;padding:16px 24px;text-align:center;"><p style="font-size:11px;color:#bbb;margin:0;">© 2026 K-DocFinder (주식회사 닥터랩) · www.kdocfinder.com</p></div></div></div></body></html>';
}